...
class FastFashion(FashionMNIST):
...
def train_uni(seed,batch_size,epochs,
    ...
    with torch.no_grad():
        ...
        np.save('res_uni/loss_fash_uni_'+params_save+'.npy',loss_uni)
        np.save('res_uni/accuracy_fash_uni_'+params_save+'.npy',accuracy_list)
        #return loss_uni
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_nes/loss_fash_nes_'+params_save+'.npy',loss_nes)
        np.save('res_nes/accuracy_fash_nes_'+params_save+'.npy',accuracy_list)
        #return loss_nes
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_avg/loss_fash_avg_'+params_save+'.npy',loss_mus)
        np.save('res_avg/accuracy_fash_avg_'+params_save+'.npy',accuracy_list)
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_sqr/loss_fash_sqr_'+params_save+'.npy',loss_mus)
        np.save('res_sqr/accuracy_fash_sqr_'+params_save+'.npy',accuracy_list)
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_abs/loss_fash_abs_'+params_save+'.npy',loss_mus)
        np.save('res_abs/accuracy_fash_abs_'+params_save+'.npy',accuracy_list)
        return None
...
############ LOAD DATASET ###################
# Load data
train_dataset = FastFashion('data/F_MNIST', train=True, download=True)
test_dataset = FastFashion('data/F_MNIST', train=False, download=True)
