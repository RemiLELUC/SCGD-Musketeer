...
class FastKMNIST(KMNIST):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Scale data to [0,1]
        self.data = self.data.unsqueeze(1).float().div(255)
        # Normalize it with the usual FasonMNIST mean and std
...
def train_uni(seed,batch_size,epochs,
    ...
    with torch.no_grad():
        ...
        np.save('res_uni/loss_kmnist_uni_'+params_save+'.npy',loss_uni)
        np.save('res_uni/accuracy_kmnist_uni_'+params_save+'.npy',accuracy_list)
        #return loss_uni
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_nes/loss_kmnist_nes_'+params_save+'.npy',loss_nes)
        np.save('res_nes/accuracy_kmnist_nes_'+params_save+'.npy',accuracy_list)
        #return loss_nes
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_avg/loss_kmnist_avg_'+params_save+'.npy',loss_mus)
        np.save('res_avg/accuracy_kmnist_avg_'+params_save+'.npy',accuracy_list)
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_sqr/loss_kmnist_sqr_'+params_save+'.npy',loss_mus)
        np.save('res_sqr/accuracy_kmnist_sqr_'+params_save+'.npy',accuracy_list)
        return None
    ...
    with torch.no_grad():
        ...
        for e in range(epochs):
            ...
            if e%10==0:
                ...
                print('Accuracy :',correct / total)
        params_save = 'seed'+str(seed)+'_lr'+str(lr)+'_epochs'+str(epochs)+'_batchsize'+str(batch_size)
        np.save('res_abs/loss_kmnist_abs_'+params_save+'.npy',loss_mus)
        np.save('res_abs/accuracy_kmnist_abs_'+params_save+'.npy',accuracy_list)
        return None
...
############ LOAD DATASET ###################
# Load data
train_dataset = FastKMNIST('data/K_MNIST', train=True, download=True)
test_dataset = FastKMNIST('data/K_MNIST', train=False, download=True)
